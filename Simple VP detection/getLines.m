...
% Set in_canny[1,2,end-1,end][:] to 0;
im_canny([1 2 end-1 end], :) = 0;

% Set in_canny[:][1,2,end-1,end] to 0;
im_canny(:, [1 2 end-1 end]) = 0;

% find the row of im_canny, i.e. height
height = size(im_canny, 1);

% find index (from top-left down to buttom-right) whose value is greater
% than 0
ind = find(im_canny > 0);

% number of direcion(four neibour and four diagonal)
num_dir = 8;

% Ontain dX and dY index
dX_ind = dX(ind);
dY_ind = dY(ind);
 
a_ind = atan(dY_ind ./ (dX_ind + 1E-10)); % Add 1E-10 to avoid #INF when atan = 1
...
% Declare a large array for storing "line" information
lines = zeros(2000, 9);
