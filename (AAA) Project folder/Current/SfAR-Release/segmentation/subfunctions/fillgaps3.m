function LS=fillgaps3(LS,athresh,dthresh)
    ...
    while lindex1<size(LS,2)
        ...
        lindex1=lindex1+1;          % Line index
        currls=LS(:,lindex1);       % Current LS
        uncheckedLS=LS(:,lindex1+1:end);    % Rest of the LS
        mergelist=zeros(1,size(uncheckedLS,2)); % 1 * no_of_uncheckedLS merge list
...
%% compute the angle, between all combintations of two ls
